...
func TestIntegrityRejectsPlausibleButSemanticallyForgedRecap(t *testing.T) {
	...
	value.Cards = cards.BuildWithRuleset(ruleset.DefaultRuleset(), value.Profile, value.Year, value.ShareID, value.Metrics, value.Behavior, value.Achievements, value.NextAction)
...
func TestIntegrityRejectsStoredAchievementsFromSameCategory(t *testing.T) {
	value := testkit.Recap()
	first := achievement.Build(model.Metrics{SalesCompleted: 5})
	second := achievement.Build(model.Metrics{ListingsPublished: 5, SalesCompleted: 1})
...
func TestIntegrityRejectsLowerGradeWhenHigherGradeWasEarned(t *testing.T) {
	...
	value.Behavior = behavior.DetectWithRuleset(ruleset.DefaultRuleset(), value.Metrics)
	expected := achievement.BuildWithRuleset(ruleset.DefaultRuleset(), value.Metrics)
	lower := achievement.Build(model.Metrics{SalesCompleted: 1})
	...
	value.NextAction = nextaction.BuildWithRuleset(ruleset.DefaultRuleset(), value.Metrics, value.ActionableState, value.Behavior)
	value.Cards = cards.BuildWithRuleset(ruleset.DefaultRuleset(), value.Profile, value.Year, value.ShareID, value.Metrics, value.Behavior, value.Achievements, value.NextAction)
	if err := structural.ValidateRecap(value); err != nil {
		t.Fatalf("fixture must be structurally valid: %v", err)
	}
